...
void ResidualBlockInfo::Evaluate() {
  ...
  // NOTE: Jacobians is just a safe keep for raw jacobians.
  // 1. I am not sure why memory needs to be allocated for it.
  // 2. Definitely helps to add a loss function on top of it.
  // 3. Raw jacobians is passed to ceres( takes ownership)
  ...
  if (loss_function) {
    ...
    loss_function->Evaluate(sq_norm, rho); // NOTE: Interesting
...
MarginalizationInfo::~MarginalizationInfo() {
  ...
  // NOTE: Since Stored as heap of memory,  need to be released
...
void MarginalizationInfo::addResidualBlockInfo(
  ...
  // NOTE: Save Address to Paramter Size Block
  for (int i = 0;
  ...
  // NOTE: Save 0 for addresses that need to be droppped.
...
void MarginalizationInfo::preMarginalize() {
  // NOTE: Evaluate Part of the problem and store the optimized parameters.
  for (auto it : factors) {
    it->Evaluate();
